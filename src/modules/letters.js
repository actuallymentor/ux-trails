...
export function measurements_to_letters( { patient_name=i18n.t( 'letters.defaultPatientName' ), labtest_scores } ) {
    ...
    return readings.map( ( { type, value, unit, day }, index ) => {
        ...
        // Jargon sin takes priority, then acronym, then confusing synonym, then default
        const test_name = enabled_sins.jargon_lab_names
            ? i18n.t( `labs.jargon.${ type }`, { defaultValue: type } )
            : enabled_sins.acronym_lab_names
